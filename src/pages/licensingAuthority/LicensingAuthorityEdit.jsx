...
} from "@services/licensingAuthority";
import { licensingAuthorityValidationSchema } from "@utils/validations";
import { ThemeContext } from "@context/ThemeContext";

const LicensingAuthorityEdit = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { theme } = useContext(ThemeContext);

  const { 
    data: licensingAuthorityData, 
    isLoading, 
    isError: isFetchError, 
    error: fetchError 
  } = useQuery({
    queryKey: ["licensing-authority", id],
  ...
  const {
    ...
    mutationFn: (data) => updateLicensingAuthority({ id, data }),
    onSuccess: () => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center transition-colors duration-200">
        <div className="text-red-500 dark:text-red-400 p-4 bg-red-50 dark:bg-red-900/20 rounded-md shadow transition-colors duration-200">
          Error loading licensing authority: {fetchError.message}
        </div>
      </div>
    );
  }
...
export default LicensingAuthorityEdit;